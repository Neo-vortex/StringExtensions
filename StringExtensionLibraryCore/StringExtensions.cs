...
namespace StringExtensionLibraryCore;
...
public static class StringExtensions
{
    ...
    public static IDictionary<string, object> JsonToDictionary(this string val)
    {
        ...
        return
            (Dictionary<string, object>)System.Text.Json.JsonSerializer.Deserialize  (val, typeof(Dictionary<string, object>));
    }
    ...
    {
        return System.Text.Json.JsonSerializer.Deserialize<ExpandoObject>(json);
    }
    ...
    public static string CreateParameters(this string value, bool useOr)
    {
        ...
        var searchParamters = value.JsonToDictionary();
        var @params = new StringBuilder("");
        if (searchParamters == null) return @params.ToString();
        for (var i = 0; i <= searchParamters.Count() - 1; i++)
        {
            var key = searchParamters.Keys.ElementAt(i);
            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                if (i < searchParamters.Count() - 1 && useOr)
                    @params.Append(" or ");
                else if (i < searchParamters.Count() - 1) @params.Append(" and ");
            }
        }
